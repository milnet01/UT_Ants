...
- ✅ [UTA-0016] **ut-ants: load a bundle and walk through it.**
  ...
  first runs for real.
  User decision (2026-09-14), asked by ut-ants-c7: the install path is
  given on the command line too, as `ut-ants <install> <bundle>`.
  docs/design.md rule 16 has the client run `ut-bake --check <install>`
...
- ✅ [UTA-0103] **Split the real-asset test file by subject, and share its System-package resolver.**
  ...
  User decision (2026-09-14): the session runs the comparison, not the
...
- ✅ [UTA-0153] **ut-ants opens at the desktop's resolution, borderless fullscreen, with a --windowed option.**
  ...
  write-code.
  Shipped 2026-09-14 in 503c3a7, green on the matrix (run 34849170271:
  GCC 14, Clang 19, MSVC). ut-ants opens fullscreen with no display mode
  set, which SDL makes borderless at the desktop's size; --windowed keeps
  the resizable 1280x720 window. SDL_WINDOW_HIGH_PIXEL_DENSITY was added
  beyond the plan so a scaled desktop keeps its real pixels. Hand runs on
  DM-Deck16][: fullscreen drew at 3840x2160, --windowed at 1280x720.
...
- ✅ [UTA-0169] **urender: lights of brightness 0 fill light clusters and shadow tiles, overflowing DM-Fetid's.**
  ...
  Progress (2026-09-17): taken by session ut-ants-66, main checkout.
  Built (2026-09-17, ut-ants-66). directLights now skips a light of
  brightness 0 unless it has a fog volume (volumeRadius not 0). Checked
  first: TriggerLight in Engine.u scales the brightness saved at BeginPlay,
  and flickerOf multiplies it, so a saved 0 never emits. The fog glow
  multiplies by brightness too. A fog volume's VolumeFog does not, and
  SurrealEngine picks fog balls by VolumeRadius alone while skipping
  brightness-0 lights for direct light, so those lights stay.
  Measured on revision-18 bakes with scratch script stats169.sh: DM-Fetid's
  overflowed clusters are gone on every view; DM-Deck16][ and AS-Frigate
  had none before or after. Frames match HEAD within one level on every
