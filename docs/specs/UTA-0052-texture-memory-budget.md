...
| § 4.6's report reaching a user | **Partial:** the report's contents are asserted by INV-10, and UTA-0011's `tests/unit/BakeCliTest.cpp` that `ut-bake` prints a `budget` object on `written` and `over-budget`. Nothing compares the printed figures with the report |
