...
| `0.4.0` | Monster Hunt | A real rotation runs — puzzle-solving bots (**S4**), map voting, friendly names, mutators, content download from the host (**S5**), per-map weapon sets a server can toggle (**S10**), and a full round played on a gamepad (**S9**) |
