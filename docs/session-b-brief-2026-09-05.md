...
you are on `session-b` and the other session is on `main`. Your build
...
worktrees, so `core.hooksPath` and the four `ants.gate.*` settings are
...
Run `./scripts/ci.sh`, then merge to `main` and push. The `pre-push` hook
gates the push either way, so nothing reaches the remote ungated — but a
local green is one leg of three, and the matrix (GCC, Clang, MSVC) is
what decides whether an item may be flipped ✅.
